...
//ORIGINAL LINE: sbyte *dt = ctime(&now);
		...
		public final int couplegiftingnew(couples[] couple, int count, gifts[] gif, int ngf) //!Gift Exchanges.
		{
			std::ofstream output = new std::ofstream();
			output.open("log.txt", std::ios_base.app);
			FILE fcc = fopen("fcalc.txt","w+");
			for (int i = 0;i < count;i++)
			{
				int[] selected = new int[ngf]; //!keeping an hash table to keep track of all the gist selected.
				for (int j = 0;j < ngf;j++)
				{
					selected[j] = 0;
				}
