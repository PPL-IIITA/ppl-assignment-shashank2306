			...
			for (int i = 0;i < ng;i++)
			{
				...
				if (girlss[i].committed == 0)
				{
					if (girlss[i].need.compare("Attractive") == 0)
					{
						...
						{
							int ss;
							ss = GlobalMembers.v.size();
							minind = GlobalMembers.v.get(tangible.RandomNumbers.nextNumber() % ss); //!Selecting random among sutable boys for a girl.
							//cout<<minind<<endl;
							GlobalMembers.v.clear();
			...
			for (int i = 0;i < count;i++) //!Using the old gifting technique, just adding the random condition.
			{
				...
				if (couple[i].btype.compare("Miser") == 0)
				{
					...
					while (true)
					{
						...
						count++;
						int j = tangible.RandomNumbers.nextNumber() % ngf;
						if (selected[j] == 1)
						{
							continue;
						}
