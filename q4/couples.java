...
//ORIGINAL LINE: sbyte *dt = ctime(&now);
		...
		public final int mosthappy(couples[] couple, int count, int k) //!bubble sort for happiness.
		{
			std::ofstream output = new std::ofstream();
			output.open("log.txt", std::ios_base.app);
			for (int i = 0;i < count;i++)
...
//ORIGINAL LINE: sbyte *dt = ctime(&now);
			...
			System.out.print("Relationships after breakups\n");
			FILE fc = fopen("New Couples.txt","w+");
			for (int i = 0;i < ng;i++) //!Printing the new Relationships.
			{
				for (int j = 0;j < nb;j++)
				{
					if (boyss[j].girlname.compareTo(girlss[i].name) == 0 && girlss[i].boyname.compareTo(boyss[j].name) == 0 && girlss[i].committed == 1 && boyss[j].committed == 1) //! They shouldn't be in a relationship prevoiusly.
					{
